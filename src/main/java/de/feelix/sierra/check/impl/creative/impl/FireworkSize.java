        ...
        if (event.getPacketType() == PacketType.Play.Client.PLAYER_BLOCK_PLACEMENT) {
            ...
            if (wrapper.getItemStack().isPresent()) {
                if (this.invalid(wrapper.getItemStack().get())) {
                    return new CrashDetails("Invalid explosion size in firework (place)", PunishType.BAN);
                }
            }
            ...
            if (wrapper.getCarriedItemStack() != null) {
                if (this.invalid(wrapper.getCarriedItemStack())) {
                    return new CrashDetails("Invalid explosion size in firework (click)", PunishType.BAN);
                }
            }
        }

        if (invalid(clickedStack)) {
            return new CrashDetails("Invalid explosion size in firework", PunishType.BAN);
        }
        return null;
    }
