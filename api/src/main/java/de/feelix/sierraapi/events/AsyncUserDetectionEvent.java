...
public class AsyncUserDetectionEvent extends Event {
     ...
     * SierraUser provides methods to retrieve information about the user, such as username, entity ID, UUID, existence since timestamp, version, and various actions and settings related
     *  to the user.
     * The SierraUser interface also provides access to the CheckRepository, which is a repository of checks in the Sierra API.
     * The SierraUser interface is implemented by various classes, including the AsyncUserDetectionEvent class.
     * Within the AsyncUserDetectionEvent class, the sierraUser variable represents the SierraUser associated with a violation.
